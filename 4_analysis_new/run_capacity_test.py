# Filename: run_capacity_test.py (Handle wait() Timeout)
import time
import datetime
import argparse
import logging
...
from dask.distributed import Client, Future, wait, TimeoutError, FIRST_COMPLETED # Keep TimeoutError
...
dask_log.setLevel(logging.WARNING) # Keep Dask logs quieter
...
if __name__ == "__main__":
    ...
    try:
        ...
        while time.time() < end_time:
            ...
            done_set = set() # Initialize default value
            try:
                if active_futures:
                    # Use string literal 'FIRST_COMPLETED'
                    done_set, _ = wait(active_futures, timeout=0.1, return_when='FIRST_COMPLETED')
            except TimeoutError:
                # This is normal if no task finished quickly
            ...
            for future in done_set:
                try:
                    ...
                    # Log error if getting result fails (e.g., task raised exception)
                    task_exception = future.exception() # Try to get original exception
