# Filename: run_capacity_test.py (Fix FIRST_COMPLETED Import)
import time
import datetime
import argparse
import logging
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"Run Dask Capacity Test (BLIP Captioning - Active Limit: {MAX_ACTIVE_FUTURES})")
    ...
    log.info(f"--- Starting Capacity Test (BLIP Captioning - Active Limit: {MAX_ACTIVE_FUTURES}) ---")
    ...
    try:
        ...
        while time.time() < end_time:
            ...
            while len(active_futures) < MAX_ACTIVE_FUTURES and time.time() < end_time:
                ...
                except StopIteration: log.warning("Reached end of Image URL file."); end_time = time.time(); break
                except Exception as e: log.error(f"Error reading URL file or submitting task: {e}", exc_info=True); time.sleep(1); break

            # Result Processing Phase
            if active_futures:
                 # --- CORRECTED wait() CALL ---
                 done_set, _ = wait(active_futures, timeout=0.1, return_when='FIRST_COMPLETED') # Use string literal
                 # --- END CORRECTION ---
            else: done_set = set()
            ...
            for future in done_set:
                try:
                    ...
                    if result and result[0] is not None:
                        ...
                        results_count += 1; processed_in_loop += 1
                    elif result and result[0] is None: log.warning(f"Task returned None ID with result: {result[1]}"); error_count += 1
                except Exception as e: log.error(f"Failed to retrieve result for completed future {future.key}: {e}", exc_info=True); error_count += 1
                finally: active_futures.remove(future) # Remove from active set
            ...
            if now - loop_start_time > 30:
                ...
                log.info(f"Submitted: {submitted_count}, Results: {results_count} (Errors: {error_count}), Active: {pending}, Rate: {rate:.2f} img/s, Elapsed: {elapsed_time/3600:.2f} hrs")
                loop_start_time = now
        ...
        if url_iterator is not None and hasattr(url_iterator, 'close'):
             if isinstance(url_iterator, io.IOBase) and not getattr(url_iterator, 'closed', True): log.info("Closing URL file."); url_iterator.close() # Check 'closed' attribute if available
        ...
        log.info(f"Requested Duration: {args.duration:.2f} hours ({test_duration_seconds:.0f} s)"); log.info(f"Actual Duration: {actual_duration / 3600:.2f} hours ({actual_duration:.0f} s)"); log.info(f"Tasks Submitted: {submitted_count}"); log.info(f"Results Collected: {results_count}"); log.info(f"Errors Reported: {error_count}"); final_pending = submitted_count - results_count - error_count; log.info(f"Tasks Pending at End (Not Processed): {final_pending}")
        if actual_duration > 0: throughput = results_count / actual_duration; log.info(f"Average Throughput (Processed Tasks): {throughput:.2f} images/sec ({throughput * 3600:.0f} images/hour)")
