# # Filename: app.py (Add MAX_CONTENT_LENGTH Debugging)
# import os
# import uuid
# import time
# import traceback
...
# --- TEMPORARY DEBUGGING app.py ---
import os
from flask import Flask, request, redirect, url_for

app = Flask(__name__)
app.config['MAX_CONTENT_LENGTH'] = 100 * 1024 * 1024
print(f"DEBUG: MAX_CONTENT_LENGTH = {app.config['MAX_CONTENT_LENGTH']}")

@app.route('/', methods=['GET'])
def index():
    # Simple form to test upload
    return '''
        <!doctype html>
        <title>Upload Test</title>
        <h1>Upload Test File</h1>
        <form method=post action="/upload_test" enctype=multipart/form-data>
...
def upload_test():
    print(f"DEBUG: Received upload request. Content-Length: {request.content_length}")
    ...
    if file:
        # Don't even save, just acknowledge success
        print(f"DEBUG: Received file: {file.filename}")
        return f'Successfully received (but not saved) file: {file.filename}', 200
    return 'Unexpected error', 500

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, debug=True) # Run with debug for more output
